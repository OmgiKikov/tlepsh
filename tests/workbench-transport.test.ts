...
describe("model-readable validation problems", () => {
	it("names the allowed grader types and task fields when a model guesses", () => {
		expect(() => WorkbenchSubmitToolSchema.prepare({
			...
			/\/tasks\/0: unknown property "id" \(allowed: input, expected, messages, simulatedUser, metadata, graders\); \/tasks\/0: unknown property "notes" \(allowed: input, expected, messages, simulatedUser, metadata, graders\); \/tasks\/0\/graders\/0: type "llm" is not supported; use one of: "tool_called" \{name\?, tool, argsContains\?\}, "output_contains" \{name\?, text, caseSensitive\?\}, "output_matches" \{name\?, pattern\}, "no_secret" \{name\?\}, "judge" \{name\?, rubric\?, assertions\?, jury\?, withReference\?\}, "exact" \{name\?, normalize\?\}, "similarity" \{name\?, metric, threshold\}/,
		);
	});
