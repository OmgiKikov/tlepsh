...
describe("side-effect-free CLI invocation parsing", () => {
	...
	it.each([
		...
		{ name: "corpus synth", argv: ["corpus", "synth", "--target", "./agent", "--sealed", "40", "--name", "exam"], command: "corpus", action: "synth" },
		{
			name: "corpus synth with a seed, a Spec and a review file",
			argv: ["corpus", "synth", "--target", "./agent", "--project", "demo", "--sealed", "40", "--name", "exam", "--seed", "s1", "--from", "spec.md", "--examples", "8", "--review", "/tmp/exam.jsonl"],
			command: "corpus",
			action: "synth",
		},
	...
	it.each([
		...
		// `corpus synth` counts cases it will WRITE, and bounds both counts.
		[["corpus", "synth", "--target", "./agent", "--sealed", "0", "--name", "x"], /--sealed .* at least 1/],
		[["corpus", "synth", "--target", "./agent", "--sealed", "201", "--name", "x"], /--sealed .* between 1 and 200/],
		[["corpus", "synth", "--target", "./agent", "--sealed", "40", "--name", "x", "--examples", "21"], /--examples .* between 0 and 20/],
		[["corpus", "synth", "--sealed", "40", "--name", "x"], /missing required flag --target for corpus synth/],
		[["corpus", "synth", "--target", "./agent", "--sealed", "40"], /missing required flag --name for corpus synth/],
		[["corpus", "synth", "--target", "./agent", "--sealed", "40", "--name", "x", "--recipe", "{}"], /unknown flag --recipe for corpus synth/],
