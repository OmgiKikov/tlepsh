...
export function loadTarget(dir: string, override?: { dataset?: string }): ResolvedTarget {
	...
	const suiteHash = hashValue({
		...
		defaults,
		// The calibration policy governs promotion, not measurement: canonical
		// JSON drops the undefined, so setting or lifting it never invalidates
		// evidence produced by the identical judge.
		judge: manifest.evalSuite.judge
			? { ...manifest.evalSuite.judge, requireCalibration: undefined }
			: null,
	});
