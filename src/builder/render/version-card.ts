...
function validationLine(card: ExecutiveVersionCard, paint: Paint): string {
	...
	return `${paint.dim("Validation")} ${paint.bold(surface)} · score ${percent(value.baseline.score)} → ${percent(value.candidate.score)} ` +
		`(${points(value.scoreDelta)}, 95% CI ${points(value.confidence95.low)} … ${points(value.confidence95.high)}) · ` +
		`pass ${percent(value.baseline.passRate)} → ${percent(value.candidate.passRate)} · ` +
...
function resourceLine(card: ExecutiveVersionCard, paint: Paint): string {
	...
	if (ratios.status === "known") {
		parts.push(`cost ${ratio(ratios.value.cost)} · latency ${ratio(ratios.value.latency)} · tokens ${ratio(ratios.value.tokens)}`);
	} else {
