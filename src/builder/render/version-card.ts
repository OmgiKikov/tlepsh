...
function validationLine(card: ExecutiveVersionCard, paint: Paint): string {
	...
	return `${paint.dim(t("version-card.validation"))} ${paint.bold(surface)} · ${t("version-card.score")} ${percent(value.baseline.score)} → ${percent(value.candidate.score)} ` +
		`(${points(value.scoreDelta)}, 95% CI ${points(value.confidence95.low)} … ${points(value.confidence95.high)}) · ` +
		`${t("version-card.pass-rate")} ${percent(value.baseline.passRate)} → ${percent(value.candidate.passRate)} · ` +
