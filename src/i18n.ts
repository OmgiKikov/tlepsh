...
const NOUNS = {
	en: {
		...
		"failing run": ["failing run", "failing runs"],
		item: ["item", "items"],
		...
		// lane: stranger-sweep
		"applied change": ["applied change", "applied changes"],
		"description draft": ["description draft", "description drafts"],
	...
	ru: {
		...
		regression: ["просадка", "просадки", "просадок"],
		"failing run": ["упавший прогон", "упавших прогона", "упавших прогонов"],
		...
		// lane: stranger-sweep
		"applied change": ["применённая правка", "применённые правки", "применённых правок"],
		"description draft": ["черновик описания", "черновика описания", "черновиков описания"],
...
const en = {
	...
	"next.improvement-authoring": "Prepare a change for the first actionable problem",
	...
	"next.candidate-checked": "Say “ship it 0.2.0” to run the exam and release — or “reject”",
	...
	"next.selection-required": "Pick which one to continue with",
	...
	"next.judge-abstained": "The judge could not decide some answers: rewrite its assertions so they check the reply text, then say “re-score”",
	"next.unstable": "Nothing fails often enough to fix: say “measure noise”, or run again with more repetitions",
	"next.errored": "The run hit errors, not failures: repair the model or sandbox path, then say “tests” again",
	"next.nothing-failed": "Everything passed: add harder cases, or supply the exam",
	// The first build: the agent is still the packaged template and a Spec to
	// build it from is approved. It lands as the working version; tests follow.
	"stage.build": "Building the agent",
	"next.build-required": "Say “build the agent” — the Builder writes it from the description and shows the diff",
	"plan.harness.template": "still the template — build it from the description",
	"welcome.build": "Build the agent from the description",
	"confirm.first-build.title": "Build the agent",
	"confirm.first-build.lead": "First build: this diff becomes the working agent.",
	"confirm.first-build.note": "Your branch fast-forwards onto this build. Nothing is measured yet, so there is nothing to compare it against and no check runs; the tests come next.",
	"message.first-build-applied": "The agent is built: {branch} @ {sha}. Write the tests next.",
	"panel.agent-built": "Agent built",
	"result.agent-built": "Agent built",
	"result.first-build-note": "This is the working agent now. Nothing was compared: there was no agent before it. Write the tests, run them, and every later change is measured against this one.",
	...
	"candidate.checked": "Change checked",
	...
	"sealed.not-executed": "not run yet — ship runs it",
	...
	"prediction.mode": "failure mode «{mode}» {from}/{of} → ≤{to}/{of}",
	...
	"confirm.exam-candidate": "Run the sealed exam on the checked change ({runs})?",
	...
	"result.target-wrapped": "Existing agent adopted",
	"result.agent-entry": "entry",
	"result.agent-harness": "editable",
	...
	"improve.best": "Best measured change · {candidate}",
	"improve.no-best": "No verified improvement found. Keep the original agent.",
	"improve.selection": "{count} changes evaluated · {charged}/{budget} executions charged",
	"improve.gain": "Score {delta} · 95% interval [{low}, {high}] · {rate} passing",
	"improve.authoring": "Authoring cases: {pass}/{total} runs passing",
	"improve.trials": "Hypotheses: {count}",
	"confirm.improve.baseline": "Baseline {target} @ {revision} · {branch}",
	"improve.uncertainty": "Selection uses the same unseen development cases. Repeated comparisons do not prove superiority; the sealed exam remains separate.",
	"improve.stop.no-progress-twice": "Two rounds without a better measured change.",
	"improve.stop.execution-budget-exhausted": "The remaining execution budget cannot cover another full round.",
	"confirm.improve.automatic": "Find and keep the best measured change; stop at the target, after two rounds without progress, or at the limit.",
	"confirm.improve.limit": "Agent execution limit: {budget}. Interrupted rounds retain their reserved charge.",
	"confirm.improve.original": "Independent changes are tested against the original agent. Review the selected diff once at the end.",
	...
	"handoff.talk-to-agent": "Want to try it? Say “open the agent” — you talk to it, and leaving it brings you back here.",
	...
	"message.exam-draft": "The judge wrote a draft exam of {cases} to a private file. The operator reads and edits it, then imports it with /holdout <path>; you never see it.",
	"message.exam-sealed-kb": "The judge wrote a sealed exam of {cases} from the knowledge base. Its content is evaluator-only and never enters this conversation.",
	"message.exam-draft-kb": "The judge wrote a draft exam of {cases} from the knowledge base to a private file. The operator reads and edits it, then imports it with /holdout <path>; you never see it.",
	"message.exam-shortfall": "{requested} were asked for; {dropped} did not survive validation ({malformed} malformed, {duplicate} duplicate) and were never part of the exam.",
	...
	"traces.fix-choice": "Fix {ordinal}: {title}",
	"traces.not-now": "Not now",
	"traces.fix-message": "Fix problem {ordinal} ({id}): {title}. Prepare the change and show me the review.",
	"holdout.path-prompt": "Path to the private sealed JSONL exam",
	"holdout.name-prompt": "Name for this immutable exam",
	"holdout.name-default": "Promotion holdout",
	"holdout.import-title": "Import sealed holdout",
	"holdout.import-question": "Import {path} as an evaluator-only exam? Builder Pi will see only whether it is large enough to ship.",
	"holdout.imported": "{count} evaluator-only cases are ready for the ship gate.",
	"holdout.hidden": "Builder Pi never receives their content or identity.",
	"holdout.imported-short": "{count} evaluator-only cases were imported; the ship gate needs {minimum} — {missing} more.",
	"holdout.import-more": "Import a sufficiently large, separate holdout before applying a candidate.",
	...
	"holdout.reason-kb": "the operator asked for a sealed exam from the knowledge base with /holdout",
	...
	"passport.exam-generated-kb": "written by the judge from the knowledge base, sealed unreviewed",
	"passport.exam-generated-kb-reviewed": "written by the judge from the knowledge base, reviewed by the operator",
	...
	"rag.title": "RAG X-ray",
	"rag.diagnosis-line": "Retrieval diagnosis: {diagnosis}",
	"rag.summary": "{searches} · Hit@k {hitAtK} · MRR {mrr} · citation {citation} · source check {grounding}",
	"rag.resources": "retrieval latency {latency} · retrieval cost {cost} · score coverage {coverage}",
	"rag.source.expected": "Expected",
	"rag.source.retrieved": "Retrieved",
	"rag.source.cited": "Cited",
	"rag.faithfulness-note": "Faithfulness: not measured. A citation or historical text-overlap pass does not prove that claims follow from the source. Chunk bodies are never included in this view.",
	"rag.search-summary": "Search {index} · {status} · k={k} · {duration}",
	"rag.query": "Query",
	"rag.search-metrics": "Hit@k {hitAtK} · reciprocal rank {rank}",
	"rag.table.chunk": "Chunk",
	"rag.table.path": "Path",
	"rag.table.score": "Score",
	"rag.table.expected": "Expected",
	"rag.table.cited": "Cited",
	"rag.table.overlap": "Answer overlap",
	"rag.no-readable-chunks": "No readable chunk metadata was recorded for this search.",
	"rag.hits-omitted": "{count} lower-ranked hit(s) omitted from this compact view.",
	"rag.no-search": "No host-verified kb_search call was recorded.",
	"rag.searches-omitted": "{searches} omitted from this compact view.",
	"rag.none": "none",
	"rag.more": "… {count} more",
	"rag.query-not-recorded": "query not recorded",
	"rag.path-missing": "path not recorded",
	"rag.score-inline": "score {score}",
	"rag.overlap-inline": "overlap {overlap}",
	"rag.flags.other": "other",
	"rag.overlap-note": "Answer overlap is token overlap, not entailment. Chunk bodies are not shown.",
	"rag.diagnosis.unlabelled": "no expected-source label",
	"rag.diagnosis.retrieval-bypassed": "retrieval bypassed",
	"rag.diagnosis.retrieval-unknown": "retrieval result unknown",
	"rag.diagnosis.retrieval-miss": "expected source not retrieved",
	"rag.diagnosis.answer-grounding-miss": "the recorded source check failed",
	"rag.diagnosis.retrieved-and-supported": "retrieved; recorded source check passed",
	"rag.diagnosis.retrieved-and-cited": "retrieved and cited",
	"rag.diagnosis.mixed": "mixed retrieval evidence",
	"rag.search-status.ok": "ok",
	"rag.search-status.error": "error",
	"rag.search-status.missing-result": "result missing",
	"rag.search-status.unreadable": "result unreadable",
	"rag.label-status.available": "labelled",
	"rag.label-status.missing": "unlabelled",
	"rag.flag.expected": "expected",
	"rag.flag.cited": "cited",
	...
	"view.known-facts": "User knows",
	"corpus.sources.current": "KB unchanged since this draft was saved; this does not verify the expected answers.",
	"corpus.sources.changed": "The KB changed since this draft was saved. Reread the sources and create a new draft with reviewed answers; renaming or editing the old draft does not refresh its source binding.",
	"corpus.sources.unknown": "This draft has no recorded KB snapshot. Source freshness is unknown: review the answers against the current KB before publishing.",
	"corpus.sources.unavailable": "Cannot verify this draft's KB snapshot. Check source access and commit pending Target changes, then retry. No tests were published or started by this step.",
	"view.user-stop": "End dialogue (not a pass verdict)",
	"view.resource.knowledge": "KB source (read-only)",
	"table.none": "No runs to show for this evaluation.",
	"table.more": "… {n} more rows · /traces next",
	"table.page": "Rows {start}-{end} of {total}",
	"table.prev": "/traces prev for earlier rows",
	...
	"table.col.mode": "what failed",
	"table.col.tools": "tools",
	"table.col.latency": "latency",
	"table.col.baseline": "baseline",
	"table.col.candidate": "candidate",
	"table.col.delta": "Δ",
	"table.col.note": "note",
	"table.cases-more": "… {n} more cases · /review next",
	"table.cases-page": "Cases {start}-{end} of {total}",
	"table.cases-prev": "/review prev for earlier cases",
	"table.excluded-infrastructure": "excluded: infrastructure",
	"table.excluded-incomplete": "excluded: incomplete repetitions",
	"candidate.by-case": "By case",
	"label.regression-guards": "Regression guards",
	"guards.kept": "{guarded} the base passed every time, all kept",
	"guards.broken": "{broken} of {guarded} the base passed every time now fail every time — cannot ship",
	"diff.new-file": "(new file)",
	"diff.deleted": "(deleted)",
	"diff.renamed-from": "(was {path})",
	...
	"panel.candidate-checked": "Change checked on the development basket",
	...
	"panel.agent-wrapped": "Agent adopted",
	...
	"trace.header": "AHDE · live run",
	"trace.open-live": "open live trace · {url}",
	"trace.grid": "{cells}  {graded}/{total} · {tally} · {elapsed}",
	"trace.sep": "·",
	"trace.activity.started": "started",
	"trace.running-more": "… {count} more running",
	"trace.last": "last: {mark} {task} · {outcome} · {passed}/{total} graders",
	...
	"view.target-missing-next": "describe the agent; the Builder creates it right here",
	...
	"view.launch": "Launch",
	"view.launch-hint": "· or just say “open the agent” and the Builder opens it for you",
	...
	"view.source-claims": "source claims (not verified):",
	"view.expected": "expected:",
	"view.world.who": "who:",
	"view.world.has": "has:",
	"view.world.wants": "wants:",
	"view.world.must": "must:",
	"trace.tool-ok": "ok",
	"trace.tool-reported": "agent-reported, not host-verified",
	"trace.no-result": "no result recorded",
	...
	"onboarding.wrap.seen": "I see an agent here ({entry}, {tools})",
	"onboarding.wrap.accept": "Adopt it",
	"onboarding.wrap.create-new": "Create a new one instead",
	"onboarding.wrap.command": "Run it as `{command}`?",
	"onboarding.wrap.command-edit": "Another command — I will type it",
	"onboarding.wrap.command-ask": "The exact command, as it would be typed",
	"onboarding.wrap.command-invalid": "That command was not changed: {reason}.",
	"onboarding.wrap.editable": "Editable files: {files}",
	"onboarding.wrap.effect": "I will add AHDE configuration and save a starting Git revision. No agent run yet.",
	"onboarding.wrap.files-edit": "Other files — I will type them",
	"onboarding.wrap.files-ask": "Paths or globs, comma separated",
	"onboarding.wrap.reason": "adopted the existing agent in this folder",
	...
	"onboarding.other-model": "Another model — I will type its id",
	...
	"why.facts": "What the traces show: {facts}",

	"mode.title.required-tool": "The required tool was never called",
	"mode.title.required-tool-named": "{tool} was never called",
	"mode.title.output-contract": "The answer missed a required element",
	"mode.title.reference-exact": "The answer did not match the reference",
	"mode.title.no-secret": "The answer leaked something shaped like a credential",
	"mode.title.semantic-rubric": "The judge's rubric was not satisfied",
	"mode.title.reference-similarity": "The answer was too far from the reference",
	"mode.title.turn-budget": "The answer took more turns than the budget",
	"mode.title.world-state": "The world did not end up the way it had to",
	"mode.title.cites-source": "The answer did not stand on the source",
	"mode.title.final-answer": "The agent returned no final answer",
	"mode.title.instability": "The same case flips between repetitions",
	"mode.title.infrastructure": "The run never reached grading",
	"mode.title.legacy": "A grader without a fingerprint failed",
	"mode.fact.no-tool-call": "no tool was called in {runs} of {observed} {runNoun}",
	"mode.fact.tool-call-as-text": "{replies} printed a tool call as text instead of making one",
	"mode.fact.asks-a-question": "{replies} asked the user a question instead of answering",
	"mode.fact.mixed-script": "{replies} mixed writing systems",
	"mode.fact.empty-reply": "{runs} of {observed} {runNoun} ended with no reply at all",
	"mode.fact.unread": "{failed} matching observation(s) failed; no trace was read for them",
	"mode.fact.nothing-visible": "{failed} matching observation(s) failed; their traces show nothing else the host can read",
	"mode.fact.infrastructure": "Execution ended before grading; this is evidence about the evaluation path, not about the agent",
	"label.impact": "Impact",
	"impact.unavailable": "unavailable — {reason}",
	"impact.targeted": "Targeted {modes}:",
	"impact.mode-rate": "baseline {baselineFailed}/{baselineTotal} failed → candidate {candidateFailed}/{candidateTotal} failed · {affected}/{source} tasks still affected",
	"impact.new-modes": "New {modes}:",
	"impact.new-mode-rate": "{category} · {tasks} · candidate {failed}/{total} failed",
	"impact.worsened-modes": "Worsened {modes}:",
	"impact.worsened-mode-rate": "{category} · baseline {baselineFailed}/{baselineTotal} failed → candidate {candidateFailed}/{candidateTotal} failed",
	"impact.task-regressions": "Task {regressions}:",
	"impact.omitted": "… +{count} more",
	"impact.inconclusive": "Inconclusive because:",
	"impact.families": "What moved, per check:",
	"impact.family-tasks": "{before}/{tasks} → {after}/{tasks} tasks pass",
	"impact.family-regressed": "· {count} newly failing",
	"impact.family-omitted": "… +{count} more checks",
	"impact.no-diagnosis": "Nothing was targeted: this candidate was built from the Spec, not from a diagnosis.",
	"label.diagnosis": "Diagnosis",
	"diagnosis.status.actionable": "actionable",
	"diagnosis.status.healthy": "healthy",
	"diagnosis.status.inconclusive": "inconclusive",
	"diagnosis.headline": "{pass}/{total} passed · {modes} failure mode(s), {systemic} of them across tasks",
	"diagnosis.infrastructure": "{errors} infrastructure error(s) made this run inconclusive; repair the evidence path and rerun.",
	"diagnosis.healthy": "No failure modes: every development case passed.",
	"diagnosis.no-explorer": "explorer link unavailable",
	"diagnosis.next.fix": "prepare a change for the first actionable problem, or choose another",
	"diagnosis.next.harder": "add harder cases, or run again to measure stability",
	"diagnosis.next.repair": "resolve the measurement problem, then run again",
	"diagnosis.mode-count": "{failed}/{total} executions failed",
	"diagnosis.details-on-request": "say “show the details” to inspect the runs",
	"mode.scope.systemic": "across tasks",
	"mode.scope.task-local": "one case",
	"mode.decision.propose": "→ propose fix",
	"mode.decision.not-selectable": "→ not selectable",
	"mode.decision.stabilize": "→ rerun to stabilize",
	"mode.decision.repair": "→ repair evidence path",
	"mode.reproduces": "reproduces {percent}%",
	"mode.more-in-explorer": "… +{count} more modes in the Evidence Explorer",
	"mode.excerpt.no-tool": "no tool call",
	"mode.excerpt.tools": "called {tools}",
	"mode.excerpt.no-reply": "no reply",
	"mode.tasks-affected": "{affected} of {total} tasks",
	"why.flip":
	...
	"confirm.title.wrap-target": "Adopt the agent already in this folder",
	...
	"confirm.title.exam-candidate": "Sealed exam of the checked change",
	...
	"measurement.on-cases-only": "on {cases}",
	...
	// lane: stranger-sweep
	"cmd.test": "Test the agent: publish and run whatever is pending",
	"cmd.fix": "Prepare the exact change for problem n",
	"cmd.ship": "Ship the verified change: promote, adopt, next cycle",
	"cmd.help": "The AHDE Builder reference",
	"cmd.doctor": "Model keys, agent readiness, and how to fix them",
	"cmd.holdout": "Get a sealed exam: your own file, the judge writes one, or “/holdout 20 from the knowledge base”",
	"cmd.status": "Where you are and what to say next",
	"cmd.run": "Run the tests, or check the change you just made",
	"cmd.calibrate": "Measure noise: the same revision against itself",
	"cmd.regrade": "Score the recorded answers again with the revised checks",
	"cmd.traces": "The diagnosis, the failure modes, and the runs table",
	"cmd.review": "What is waiting for your review, with its actions",
	"cmd.approve": "Approve the reviewed description",
	"cmd.publish": "Publish the reviewed test cases",
	"cmd.apply": "Apply the reviewed change to a candidate branch",
	"cmd.discard": "Discard the change, or drop an interrupted candidate",
	"cmd.promote": "Ship the checked candidate",
	"cmd.reject": "Reject the checked candidate",
	"cmd.adopt": "Move your branch onto the shipped candidate",
	"cmd.next": "Close this cycle and continue with the active agent",
	"cmd.target": "The exact committed agent, or one of its resources",
	"cmd.passport": "What a shipped version promised and measured",
	"version-card.title": "VERSION CARD · {agent} {version}",
	"version-card.decision.improvement-proved": "The sealed exam proved an improvement",
	"version-card.decision.no-regression-proved": "The sealed exam proved no regression",
	"version-card.decision.sealed-pass-unknown": "The sealed exam passed; its conclusion is unknown",
	"version-card.decision.sealed-failed": "The sealed exam failed",
	"version-card.decision.sealed-inconclusive": "The sealed exam is inconclusive",
	"version-card.decision.sealed-unknown": "The sealed conclusion is unknown",
	"version-card.unknown": "unknown ({reason})",
	"version-card.validation": "Validation",
	"version-card.surface.blind-validation": "blind validation",
	"version-card.surface.development": "development",
	"version-card.surface.unknown": "validation surface unknown",
	"version-card.score": "score",
	"version-card.pass-rate": "pass rate",
	"version-card.excluded": "{count} excluded",
	"version-card.sealed": "Sealed exam",
	"version-card.sealed.improved": "improvement proved",
	"version-card.sealed.no-regression": "no regression proved",
	"version-card.sealed.origin-unknown": "origin unknown",
	"version-card.capabilities": "Capabilities",
	"version-card.check.required-tool": "Required tool call",
	"version-card.check.output-contains": "Required answer content",
	"version-card.check.output-matches": "Answer format",
	"version-card.check.no-secret": "No exposed secrets",
	"version-card.check.semantic-rubric": "Judge rubric",
	"version-card.check.reference-exact": "Exact reference match",
	"version-card.check.reference-similarity": "Reference similarity",
	"version-card.check.turn-budget": "Conversation length",
	"version-card.check.world-state": "Resulting world state",
	"version-card.check.cites-source": "Recorded source check",
	"version-card.check.final-answer": "Final answer provided",
	"version-card.none-measured": "none measured",
	"version-card.more": "{count} more",
	"version-card.regressions": "Regressions",
	"version-card.regression-summary": "{tasks} task regressions · {newModes} new modes · {worsened} worsened modes · {unresolved} targeted unresolved",
	"version-card.resources": "Resources",
	"version-card.absolute": "absolute",
	"version-card.ratios": "ratios",
	"version-card.cost": "cost",
	"version-card.latency": "latency",
	"version-card.tokens": "tokens",
	"version-card.change": "Change",
	"version-card.paths": "Paths",
	"version-card.artifacts": "Artifacts",
	"version-card.passport": "passport",
	"version-card.dataset": "dataset",
	"version-card.dialogues": "{count} dialogues",
	"version-card.warnings": "Warnings",
	"release.html.report": "RELEASE REPORT",
	"release.html.before": "Before",
	"release.html.after": "After",
	"release.html.delta": "Score change",
	"release.html.confidence": "95% confidence interval",
	"release.html.sealed-note": "The exam is separate from development. Its questions remain private.",
	"release.html.regressed-cases": "Regressed cases",
	"release.html.new-failures": "New failure types",
	"release.html.unresolved": "Unresolved targets",
	"release.html.worsened": "Worsened failure types: {count}",
	"release.html.total-cost": "Total run cost · before → after",
	"release.html.mean-latency": "Mean latency · before → after",
	"release.html.per-answer": "Per answer, after / before",
	"release.html.exact-diff": "Inspect the exact change",
	"release.html.evidence-bound": "Recorded evidence for this version",
	"release.html.artifacts-note": "The report opens on its own. To use the download links, keep it in exports/ beside the original project artifacts.",
	"release.html.revisions": "Exact revisions · before → after",
	"release.html.offline": "Works offline. Print from your browser to save a PDF.",
	"release.html.saved": "Shareable report: {path}",
	"release.html.not-saved": "The shareable report could not be saved; the version evidence remains available above.",
	...
	"cmd.label": "Check the judge: grade its answers blind, up to {max} at a time",
	"cmd.err.tui": "/{command} works only in the Builder window",
	"cmd.err.no-args": "/{command} takes no arguments",
	"cmd.err.repetitions": "/{command} takes how many repetitions — a whole number from 1 to 10",
	"cmd.err.regrade-id": "/regrade takes the run id /traces shows, for example /regrade erun_abc123",
	"cmd.err.branch": "a branch name may hold only letters, digits and . _ - /",
	...
	"cmd.err.no-problem": "There is no problem {ordinal} to fix; /traces lists {count}.",
	"cmd.err.holdout-count": "How many cases has to be a whole number; you typed {answer}",
	"cmd.err.holdout-unavailable": "Importing a sealed exam is not available in this window",
	"cmd.err.traces-rows": "/traces takes a row count (up to 60 per page), next or prev; for example /traces 30",
	"cmd.err.review-page": "/review takes no arguments, next or prev for candidate case pages",
	"cmd.review-no-cases": "No readable candidate comparison to page through here. Use /review to inspect the current subject.",
	"cmd.review-no-more": "No more candidate cases in that direction.",
	"cmd.err.target-arg": "/target takes at most one resource path of the agent",
	"cmd.err.passport-arg": "/passport takes at most one version, for example /passport 0.2.0",
	"cmd.err.log-rows": "/log takes how many rows to show, for example /log 10",
	"cmd.err.label-count": "/label takes how many answers to grade, for example /label 20 (1 to {max})",
	"cmd.err.label-host": "/label needs a window that can ask you questions; this one cannot",
	"cmd.nothing-fixable": "No failure mode has enough evidence to change the agent yet. Run the tests again, or add cases.",
	"cmd.nothing-to-fix": "Nothing to fix yet — {next}",
	"cmd.ask-fix": "Say: “fix problem {ordinal}”.",
	"cmd.passport-not-writable": "This folder is read-only, so nothing was saved beside the agent.",
	"reason.interactive": "Requested interactively via /{command}",
	"model.auth-rejected": "The Builder's model refused the key. Say “connect model” to set it up again, then repeat.",
	"model.rate-limited": "The Builder's model is rate-limited. Wait and repeat, or say “connect model” to pick another one.",
	"model.unreachable": "The Builder's model is unreachable. Check the network, or say “connect model” to pick another one.",
	"model.failed": "The Builder's model did not answer. Repeat, or say “connect model” to set it up again.",
	"model.not-found": "The Builder's model or its endpoint was not found (404). Say “connect model” and pick another model or provider.",
	"header.model-not-connected": "Builder model not connected",
	"onboarding.folder-not-empty": "This folder already holds {found}, so the agent cannot be created here. Open an empty folder and start again.",
	"onboarding.folder-unusable": "The agent cannot live in this folder — it is missing, or it is not an ordinary folder. Open a normal, empty project folder.",
	"onboarding.setup-stopped": "Setup stopped. Tell me when you want to pick the agent's model.",
	"onboarding.setup-failed": "Setup did not finish: {reason}",
	"onboarding.credential-name-only": "Give the NAME of the environment variable that holds the key, never the key itself.",
	"extension.no-shell": "The Builder has no shell here. Say in words what you need done and it does it with its own tools.",
	"blocker.target-missing": "The agent has not been created yet.",
	"blocker.target-placeholder": "The agent still carries the template's stand-in name and model.",
	"blocker.candidates-ambiguous": "{candidates} fit this project — pick one.",
	"blocker.interrupted-candidate": "The interrupted attempt has to be dropped before a new one starts.",
	"blocker.terminal-candidates": "{candidates} are finished but not yet adopted or closed.",
	"blocker.applied-without-evidence": "{proposals} are applied but have no evidence yet.",
	"blocker.proposals-await": "{proposals} are waiting for a decision.",
	"blocker.spec-drafts": "{drafts} are waiting for review.",
	"blocker.approved-specs": "{specs} exist — pick the one to continue with.",
	"blocker.corpus-drafts": "{drafts} match the approved description — pick one.",
	...
	"refusal.target-not-ready": "The agent is not ready yet. {next}",
	"artifact.approved-spec": "approved description",
	...
	"view.resource.instructions": "instructions",
	"view.resource.tool-executable": "tool program",
	"view.resource.tool-descriptor": "tool description",
	"view.resource.skill": "skill",
	"view.resource.harness-file": "harness file",
	...
	"confirm.improve.target-subtotal": "Target subtotal",
	"confirm.improve.builder-ceiling": "Builder ceiling",
...
one would cost more than usual, and then you get a single question.`,
	...
	"why.expected.explicit-citation": "expected an explicit citation of {source}",
	"why.actual.explicit-citation": "the final answer contains no exact source id; matching text is not a citation",
	...
	"refusal.regression-guards-broken": "The change breaks {broken} of {guarded} development cases the base version passed every time. Fix it, or verify it again with more repetitions; a broken guard cannot ship.",
	"refusal.ship-check-regressed": "The check regressed on the development basket; a regressed change cannot ship. Fix it, or reject it.",
	...
	// lane: the basket — coverage, the critic, the reading after a run, the simulator
	"mode.title.output-excludes": "The answer said something it must not",
	"job.label.critique": "the case review",
	"confirm.title.critique-corpus": "Review the cases with the judge",
	"confirm.critique-corpus": "Ask the judge to read {cases} for validity ({calls})? It sees the cases and their sources, never the agent's answers.",
	"panel.critic": "Case review",
	"critic.summary": "{valid} valid · {repair} to repair · {invalid} invalid · {unreviewed} unreviewed",
	"critic.verdict.valid": "valid",
	"critic.verdict.repair": "repair",
	"critic.verdict.invalid": "invalid",
	"critic.verdict.unreviewed": "unreviewed",
	"critic.none": "the critic has not read this draft",
	"critic.next": "Repair the cases marked to repair, exclude the invalid ones with a reason, then test.",
	"critic.exam-dropped": "{dropped} generated case(s) failed the critic and were not sealed: {reasons}",
	"refusal.publish-invalid-cases": "The critic marked {invalid} case(s) invalid. Repair them or exclude them with a reason; publish with force to override, on record.",
	"coverage.title": "Coverage",
	"coverage.row": "{job}: {cells}",
	"coverage.empty": "empty: {cells}",
	"coverage.unlabelled": "{cases} without a coverage label",
	"coverage.unknown-job": "not a Spec job",
	"coverage.origins": "origin: {real} real · {synthetic} synthetic · {unknown} unlabelled",
	"coverage.excluded": "excluded with a reason: {cases}",
	"coverage.exam": "coverage: {labelled} labelled · {unlabelled} without a cell",
	"basket.title": "Basket after this run",
	"basket.saturated": "{cases} passed every time — regression checks; the next wave goes harder",
	"basket.failing-valid": "{cases} failed every time and the critic found the case sound — capability work for the agent",
	"basket.failing-doubtful": "{cases} failed every time and the critic doubts the test itself — review its criteria and conditions, never drop it for failing",
	"basket.failing-unreviewed": "{cases} failed every time and nobody has read the case — run the critic before deciding anything",
	"basket.unstable": "{cases} moved between repetitions — noise, not a verdict",
	"basket.wave": "this wave added {cases}, {failing} of them fail",
	"basket.wave-saturated": "this wave added {cases} and the agent passed them all: saturation — the next wave must be harder",
	"basket.origins": "origins: {real} real · {synthetic} synthetic · {unknown} unlabelled; realism {realism}",
	"basket.realism.unverified": "unverified — no comparable real cases yet",
	"basket.realism.compared": "compared on {cells} comparable cell(s)",
	"basket.origin-cell": "{cell}: real {real} vs synthetic {synthetic} ({gap})",
	"basket.next-wave": "next wave: {targets}",
	"basket.next-wave.cells": "empty cells {cells}",
	"basket.next-wave.modes": "unresolved modes {modes}",
	"basket.next-wave.harder": "harder cases for {jobs}",
	"basket.rule": "A case is never removed for failing; only an invalid test is repaired or excluded, with its reason.",
	"noise.simulator": "simulator noise: the second arm used {model}; this band is about the user model, never the agent, and it is not shipping evidence",
	"confirm.calibrate.simulator": "Run the same revision against itself with the alternate user model {model} on the second arm ({runs})?",
	"behavior.clear": "clear",
	"behavior.vague": "vague",
	"behavior.impatient": "impatient",
	"behavior.wrong-facts": "wrong facts",
	"behavior.changes-goal": "changes goal",
	"behavior.multi-issue": "several issues",
	"behavior.terse": "terse",
	"behavior.non-native": "non-native",
} as const;
...
const ru: Record<MessageKey, string> = {
	...
	"next.improvement-authoring": "Подготовить правку для первой проблемы, которую можно исправить",
	"next.proposal-review": "Прочитай диф и скажи «применить» или «выброси»",
	"next.candidate-verification": "Скажи «проверь» — проверю правку",
	"next.candidate-checked": "Скажи «выкатывай 0.2.0» — прогоню экзамен и выпущу; или «отклонить»",
	...
	"next.judge-abstained": "Судья не смог решить часть ответов: перепиши его утверждения так, чтобы они проверялись по тексту ответа, и скажи «пересчитай»",
	"next.unstable": "Ни один сбой не повторяется достаточно, чтобы его чинить: скажи «измерь шум» или прогони ещё раз с большим числом повторов",
	"next.errored": "Прогон упёрся в ошибки, а не в провалы: почини модель или песочницу и скажи «тесты» ещё раз",
	"next.nothing-failed": "Всё прошло: добавь кейсы посложнее или дай экзамен",
	"stage.build": "Сборка агента",
	"next.build-required": "Скажи «собери агента» — напишу его по описанию и покажу дифф",
	"plan.harness.template": "ещё шаблон — собери по описанию",
	"welcome.build": "Собери агента по описанию",
	"confirm.first-build.title": "Собрать агента",
	"confirm.first-build.lead": "Первая сборка: эта правка станет рабочим агентом.",
	"confirm.first-build.note": "Твоя ветка перейдёт на эту сборку. Измерять пока нечего — сравнивать не с чем, проверки не будет; дальше тесты.",
	"message.first-build-applied": "Агент собран: {branch} @ {sha}. Дальше — тесты.",
	"panel.agent-built": "Агент собран",
	"result.agent-built": "Агент собран",
	"result.first-build-note": "Теперь это рабочий агент. Сравнивать было не с чем — до него агента не было. Напиши тесты, прогони их, и каждая следующая правка измеряется относительно этой.",
	...
	"verdict.no-change": "без изменений",
	...
	"candidate.checked": "Правка проверена",
	...
	"sealed.not-executed": "ещё не прогонялся — прогонит выкатка",
	...
	"prediction.mode": "тип сбоя «{mode}» {from}/{of} → ≤{to}/{of}",
	...
	"confirm.exam-candidate": "Прогнать закрытый экзамен на проверенной правке ({runs})?",
	...
	"result.agent-entry": "запуск",
	"result.agent-harness": "правим",
	...
	"improve.best": "Лучшее измеренное изменение · {candidate}",
	"improve.no-best": "Подтверждённого улучшения нет. Сохраняем исходного агента.",
	"improve.selection": "Проверено изменений: {count} · учтено прогонов: {charged}/{budget}",
	"improve.gain": "Оценка {delta} · интервал 95% [{low}, {high}] · успешных {rate}",
	"improve.authoring": "Кейсы автора: успешных прогонов {pass}/{total}",
	"improve.trials": "Гипотез: {count}",
	"confirm.improve.baseline": "Исходный агент {target} @ {revision} · {branch}",
	"improve.uncertainty": "Выбор сделан на одних скрытых от автора проверочных кейсах. Повторные сравнения не доказывают превосходство; закрытый экзамен остаётся отдельным.",
	"improve.stop.no-progress-twice": "Два раунда без лучшего измеренного результата.",
	"improve.stop.execution-budget-exhausted": "Остатка лимита не хватает на следующий полный раунд.",
	"confirm.improve.automatic": "Найду и сохраню лучшее измеренное изменение; остановлюсь по цели, после двух раундов без прогресса или по лимиту.",
	"confirm.improve.limit": "Лимит прогонов агента: {budget}. Прерванные раунды сохраняют зарезервированный расход.",
	"confirm.improve.original": "Независимые изменения проверяются против исходного агента. Выбранный дифф рассмотрим один раз в конце.",
	...
	"handoff.talk-to-agent": "Хочешь попробовать? Скажи «открой агента» — поговоришь с ним, а выйдешь и вернёшься сюда.",
	...
	"message.exam-draft": "Судья написал черновик экзамена на {cases} в приватный файл. Оператор его читает и правит, потом загружает командой /holdout <путь>; ты его не увидишь.",
	"message.exam-sealed-kb": "Судья написал закрытый экзамен на {cases} по базе знаний. Содержимое видит только оценщик — в этот разговор оно не попадает.",
	"message.exam-draft-kb": "Судья написал черновик экзамена на {cases} по базе знаний в приватный файл. Оператор его читает и правит, потом загружает командой /holdout <путь>; ты его не увидишь.",
	"message.exam-shortfall": "Просили {requested}; {dropped} не прошли проверку ({malformed} с ошибками формы, {duplicate} дубликатов) и в экзамен не попали.",
	...
	"traces.fix-choice": "Починить {ordinal}: {title}",
	"traces.not-now": "Не сейчас",
	"traces.fix-message": "Почини проблему {ordinal} ({id}): {title}. Подготовь правку и покажи её на обзор.",
	"holdout.path-prompt": "Путь к приватному JSONL-экзамену",
	"holdout.name-prompt": "Как назвать этот экзамен (он неизменяемый)",
	"holdout.name-default": "Экзамен для выкатки",
	"holdout.import-title": "Загрузить закрытый экзамен",
	"holdout.import-question": "Загрузить {path} как экзамен только для оценщика? Билдер узнает лишь, хватает ли его для выкатки.",
	"holdout.imported": "Кейсов только для оценщика: {count} — порог выкатки готов.",
	"holdout.hidden": "Билдер не получает ни их содержимого, ни их идентичности.",
	"holdout.imported-short": "Загружено кейсов только для оценщика: {count}; для выкатки нужно {minimum} — ещё {missing}.",
	"holdout.import-more": "Загрузи достаточно большой отдельный экзамен до того, как применять кандидата.",
	...
	"holdout.reason-kb": "оператор попросил закрытый экзамен по базе знаний через /holdout",
	...
	"passport.exam-generated-kb": "написан судьёй по базе знаний, закрыт без проверки",
	"passport.exam-generated-kb-reviewed": "написан судьёй по базе знаний, проверен оператором",
	...
	"rag.title": "RAG: рентген поиска",
	"rag.diagnosis-line": "Диагноз поиска: {diagnosis}",
	"rag.summary": "{searches} · попадание@k {hitAtK} · MRR {mrr} · цитирование {citation} · проверка источника {grounding}",
	"rag.resources": "время поиска {latency} · цена поиска {cost} · оценки ранжирования {coverage}",
	"rag.source.expected": "Ожидались",
	"rag.source.retrieved": "Найдены",
	"rag.source.cited": "Упомянуты",
	"rag.faithfulness-note": "Обоснованность утверждений источником не измерялась. Ссылка или историческое совпадение текста этого не доказывают. Тексты фрагментов здесь не показываются.",
	"rag.search-summary": "Поиск {index} · {status} · k={k} · {duration}",
	"rag.query": "Запрос",
	"rag.search-metrics": "попадание@k {hitAtK} · обратный ранг {rank}",
	"rag.table.chunk": "Фрагмент",
	"rag.table.path": "Путь",
	"rag.table.score": "Оценка",
	"rag.table.expected": "Ожидался",
	"rag.table.cited": "Упомянут",
	"rag.table.overlap": "Пересечение с ответом",
	"rag.no-readable-chunks": "Для этого поиска не записаны читаемые метаданные фрагментов.",
	"rag.hits-omitted": "В компактном виде не показаны фрагменты ниже в выдаче: {count}.",
	"rag.no-search": "Подтверждённого хостом поиска по базе знаний не записано.",
	"rag.searches-omitted": "В компактном виде не показано: {searches}.",
	"rag.none": "нет",
	"rag.more": "… ещё {count}",
	"rag.query-not-recorded": "запрос не записан",
	"rag.path-missing": "путь не записан",
	"rag.score-inline": "оценка {score}",
	"rag.overlap-inline": "пересечение {overlap}",
	"rag.flags.other": "прочее",
	"rag.overlap-note": "Пересечение ответа — это совпадение токенов, а не доказательство смысла. Тексты фрагментов не показываются.",
	"rag.diagnosis.unlabelled": "нет разметки ожидаемого источника",
	"rag.diagnosis.retrieval-bypassed": "поиск пропущен",
	"rag.diagnosis.retrieval-unknown": "результат поиска неизвестен",
	"rag.diagnosis.retrieval-miss": "ожидаемый источник не найден",
	"rag.diagnosis.answer-grounding-miss": "записанная проверка источника не пройдена",
	"rag.diagnosis.retrieved-and-supported": "источник найден; записанная проверка пройдена",
	"rag.diagnosis.retrieved-and-cited": "источник найден и указан",
	"rag.diagnosis.mixed": "смешанные данные поиска",
	"rag.search-status.ok": "готово",
	"rag.search-status.error": "ошибка",
	"rag.search-status.missing-result": "результат не записан",
	"rag.search-status.unreadable": "результат не читается",
	"rag.label-status.available": "есть разметка",
	"rag.label-status.missing": "без разметки",
	"rag.flag.expected": "ожидался",
	"rag.flag.cited": "указан",
	...
	"view.known-facts": "Пользователь знает",
	"corpus.sources.current": "База знаний не менялась с сохранения черновика; это не подтверждает правильность эталонных ответов.",
	"corpus.sources.changed": "База знаний изменилась после сохранения черновика. Перечитай источники и создай новый черновик с проверенными ответами; переименование или правка старого не обновляет его привязку к источникам.",
	"corpus.sources.unknown": "У черновика нет сохранённого снимка базы знаний. Актуальность источников неизвестна: перед публикацией сверь ответы с текущей базой знаний.",
	"corpus.sources.unavailable": "Не удалось проверить снимок базы знаний черновика. Проверь доступ к источникам и зафиксируй изменения агента, затем повтори. На этом шаге тесты не публиковались и не запускались.",
	"view.user-stop": "Конец диалога (не вердикт успеха)",
	"view.resource.knowledge": "Источник знаний (только чтение)",
	"table.none": "В этой оценке нет прогонов.",
	"table.more": "… ещё {n} строк · /traces next",
	"table.page": "Строки {start}-{end} из {total}",
	"table.prev": "/traces prev для предыдущих строк",
	...
	"table.col.mode": "что сломалось",
	"table.col.tools": "тулы",
	"table.col.latency": "время",
	"table.col.baseline": "было",
	"table.col.candidate": "стало",
	"table.col.delta": "Δ",
	"table.col.note": "примечание",
	"table.cases-more": "… ещё {n} кейсов · /review next",
	"table.cases-page": "Кейсы {start}-{end} из {total}",
	"table.cases-prev": "/review prev для предыдущих кейсов",
	"table.excluded-infrastructure": "исключён: инфраструктура",
	"table.excluded-incomplete": "исключён: неполные повторения",
	"candidate.by-case": "По кейсам",
	"label.regression-guards": "Регрессионные кейсы",
	"guards.kept": "{guarded} база проходила всегда, все удержаны",
	"guards.broken": "сломано {broken} из {guarded}, которые база проходила всегда: так не выкатить",
	"diff.new-file": "(новый файл)",
	"diff.deleted": "(удалён)",
	"diff.renamed-from": "(был {path})",
	"diff.unchanged": "без изменений: {n} строк",
	...
	"panel.candidate-checked": "Правка проверена на корзине разработки",
	...
	"panel.agent-wrapped": "Агент принят",
	...
	"trace.header": "AHDE · прогон вживую",
	"trace.open-live": "открыть живой трейс · {url}",
	"trace.grid": "{cells}  {graded}/{total} · {tally} · {elapsed}",
	"trace.sep": "·",
	"trace.activity.started": "старт",
	"trace.running-more": "… ещё выполняются: {count}",
	"trace.last": "последний: {mark} {task} · {outcome} · проверок {passed}/{total}",
	...
	"view.target-missing-next": "опиши агента — Билдер создаст его прямо здесь",
	...
	"view.launch": "Запуск",
	"view.launch-hint": "· или просто скажи «открой агента» — Билдер откроет сам",
	...
	"view.source-claims": "источники со слов автора (не проверены):",
	"view.expected": "ожидается:",
	"view.world.who": "кто:",
	"view.world.has": "что есть:",
	"view.world.wants": "что хочет:",
	"view.world.must": "что должно:",
	"trace.tool-ok": "ок",
	"trace.tool-reported": "со слов агента, не подтверждено хостом",
	"trace.no-result": "результат не записан",
	...
	"onboarding.wrap.seen": "Вижу агента ({entry}, {tools})",
	"onboarding.wrap.accept": "Принять",
	"onboarding.wrap.create-new": "Создать новый",
	"onboarding.wrap.command": "Запускать как `{command}`?",
	"onboarding.wrap.command-edit": "Другая команда — впишу сам",
	"onboarding.wrap.command-ask": "Точная команда, как её набирают в терминале",
	"onboarding.wrap.command-invalid": "Команда не изменена: {reason}.",
	"onboarding.wrap.editable": "Можно править: {files}",
	"onboarding.wrap.effect": "Добавлю конфигурацию AHDE и сохраню стартовую Git-ревизию. Агент пока не запускается.",
	"onboarding.wrap.files-edit": "Другие файлы — впишу сам",
	"onboarding.wrap.files-ask": "Пути или маски через запятую",
	"onboarding.wrap.reason": "принял агента, который уже лежал в этой папке",
	...
	"onboarding.other-model": "Другую модель — введу id",
	...
	"why.facts": "Что видно в трейсах: {facts}",

	"mode.title.required-tool": "Нужный инструмент так и не вызван",
	"mode.title.required-tool-named": "{tool} так и не вызван",
	"mode.title.output-contract": "В ответе нет обязательного элемента",
	"mode.title.reference-exact": "Ответ не совпал с эталоном",
	"mode.title.no-secret": "В ответе утекло что-то похожее на секрет",
	"mode.title.semantic-rubric": "Критерий судьи не выполнен",
	"mode.title.reference-similarity": "Ответ слишком далёк от эталона",
	"mode.title.turn-budget": "Ответ занял больше ходов, чем отведено",
	"mode.title.world-state": "Мир не пришёл в нужное состояние",
	"mode.title.cites-source": "Ответ не опирается на источник",
	"mode.title.final-answer": "Агент не дал итогового ответа",
	"mode.title.instability": "Один и тот же кейс скачет между повторами",
	"mode.title.infrastructure": "Прогон не дошёл до проверки",
	"mode.title.legacy": "Провалилась проверка без отпечатка",
	"mode.fact.no-tool-call": "ни одного вызова инструмента в {runs} из {observed} {runNoun}",
	"mode.fact.tool-call-as-text": "вызов инструмента напечатан текстом вместо вызова — {replies}",
	"mode.fact.asks-a-question": "агент задаёт вопрос вместо ответа — {replies}",
	"mode.fact.mixed-script": "смешаны системы письма — {replies}",
	"mode.fact.empty-reply": "{runs} из {observed} {runNoun} закончились без ответа",
	"mode.fact.unread": "провалено совпадающих наблюдений: {failed}; трейсы по ним не читались",
	"mode.fact.nothing-visible": "провалено совпадающих наблюдений: {failed}; в трейсах больше нет ничего, что хост читает детерминированно",
	"mode.fact.infrastructure": "Выполнение оборвалось до проверки; это свидетельство о тракте оценки, а не о поведении агента",
	"label.impact": "Итог",
	"impact.unavailable": "недоступен — {reason}",
	"impact.targeted": "Цели ({modes}):",
	"impact.mode-rate": "до: провалено {baselineFailed}/{baselineTotal} → после: {candidateFailed}/{candidateTotal} · задач ещё задето {affected}/{source}",
	"impact.new-modes": "Новые типы сбоя ({modes}):",
	"impact.new-mode-rate": "{category} · {tasks} · у кандидата провалено {failed}/{total}",
	"impact.worsened-modes": "Стало хуже ({modes}):",
	"impact.worsened-mode-rate": "{category} · до: провалено {baselineFailed}/{baselineTotal} → после: {candidateFailed}/{candidateTotal}",
	"impact.task-regressions": "Просевшие задачи ({regressions}):",
	"impact.omitted": "… ещё: {count}",
	"impact.inconclusive": "Неубедительно, потому что:",
	"impact.families": "Что сдвинулось, по проверкам:",
	"impact.family-tasks": "проходит задач {before}/{tasks} → {after}/{tasks}",
	"impact.family-regressed": "· новых падений: {count}",
	"impact.family-omitted": "… ещё проверок: {count}",
	"impact.no-diagnosis": "Цели не было: этот кандидат собран по описанию, а не по диагнозу.",
	"label.diagnosis": "Диагноз",
	"diagnosis.status.actionable": "есть за что взяться",
	"diagnosis.status.healthy": "всё чисто",
	"diagnosis.status.inconclusive": "неубедительно",
	"diagnosis.headline": "пройдено {pass}/{total} · типов сбоя: {modes}, из них по нескольким задачам: {systemic}",
	...
	"diagnosis.next.fix": "подготовить правку для первой подходящей проблемы или выбрать другую",
	"diagnosis.next.harder": "добавь кейсы посложнее или прогони ещё раз — померим стабильность",
	"diagnosis.next.repair": "устранить проблему измерения и запустить проверку заново",
	"diagnosis.mode-count": "сбой в {failed}/{total} прогонах",
	"diagnosis.details-on-request": "скажи «покажи подробности», чтобы разобрать прогоны",
	"mode.scope.systemic": "по нескольким задачам",
	"mode.scope.task-local": "один кейс",
	"mode.decision.propose": "→ можно чинить",
	"mode.decision.not-selectable": "→ не выбрать",
	"mode.decision.stabilize": "→ перезапустить ради стабильности",
	"mode.decision.repair": "→ починить тракт данных",
	"mode.reproduces": "воспроизводится {percent}%",
	"mode.more-in-explorer": "… ещё режимов в проводнике: {count}",
	"mode.excerpt.no-tool": "без вызова инструментов",
	"mode.excerpt.tools": "вызвал {tools}",
	"mode.excerpt.no-reply": "без ответа",
	"mode.tasks-affected": "задач: {affected} из {total}",
	"why.flip":
	...
	"confirm.title.wrap-target": "Принять агента, который уже в этой папке",
	...
	"confirm.title.exam-candidate": "Закрытый экзамен проверенной правки",
	...
	"measurement.on-cases-only": "на {cases}",
	...
	// lane: stranger-sweep
	"cmd.test": "Проверить агента: опубликовать и прогнать всё, что ждёт",
	"cmd.fix": "Подготовить точную правку по проблеме n",
	"cmd.ship": "Выкатить проверенную правку: выкатка, принятие, новый цикл",
	"cmd.help": "Справка AHDE Билдера",
	"cmd.doctor": "Ключи моделей, готовность агента и как починить",
	"cmd.holdout": "Закрытый экзамен: твой файл, судья напишет или «/holdout 20 из базы знаний»",
	"cmd.status": "Где ты и что сказать дальше",
	"cmd.run": "Прогнать тесты или проверить только что сделанную правку",
	"cmd.calibrate": "Измерить шум: та же ревизия против себя же",
	"cmd.regrade": "Пересчитать записанные ответы новыми проверками",
	"cmd.traces": "Разбор, типы сбоев и таблица прогонов",
	"cmd.review": "Что ждёт твоей проверки, вместе с действиями",
	"cmd.approve": "Одобрить проверенное описание",
	"cmd.publish": "Опубликовать проверенные тесты",
	"cmd.apply": "Применить проверенную правку на ветку кандидата",
	"cmd.discard": "Выбросить правку или сбросить прерванного кандидата",
	"cmd.promote": "Выкатить проверенного кандидата",
	"cmd.reject": "Отклонить проверенного кандидата",
	"cmd.adopt": "Перевести твою ветку на выкаченного кандидата",
	"cmd.next": "Закрыть цикл и продолжить с активным агентом",
	"cmd.target": "Точный закоммиченный агент или один его ресурс",
	"cmd.passport": "Что обещала и что измерила выкаченная версия",
	"version-card.title": "КАРТОЧКА ВЕРСИИ · {agent} {version}",
	"version-card.decision.improvement-proved": "Закрытый экзамен доказал улучшение",
	"version-card.decision.no-regression-proved": "Закрытый экзамен доказал отсутствие регресса",
	"version-card.decision.sealed-pass-unknown": "Закрытый экзамен пройден, вывод неизвестен",
	"version-card.decision.sealed-failed": "Закрытый экзамен провален",
	"version-card.decision.sealed-inconclusive": "Закрытый экзамен не дал вывода",
	"version-card.decision.sealed-unknown": "Вывод закрытого экзамена неизвестен",
	"version-card.unknown": "неизвестно ({reason})",
	"version-card.validation": "Проверка",
	"version-card.surface.blind-validation": "слепая выборка",
	"version-card.surface.development": "набор разработки",
	"version-card.surface.unknown": "выборка проверки неизвестна",
	"version-card.score": "балл",
	"version-card.pass-rate": "успешные ответы",
	"version-card.excluded": "исключено: {count}",
	"version-card.sealed": "Закрытый экзамен",
	"version-card.sealed.improved": "улучшение доказано",
	"version-card.sealed.no-regression": "отсутствие регресса доказано",
	"version-card.sealed.origin-unknown": "происхождение неизвестно",
	"version-card.capabilities": "Возможности",
	"version-card.check.required-tool": "Вызов нужного инструмента",
	"version-card.check.output-contains": "Нужный текст в ответе",
	"version-card.check.output-matches": "Формат ответа",
	"version-card.check.no-secret": "Отсутствие утечки секретов",
	"version-card.check.semantic-rubric": "Оценка судьёй по критериям",
	"version-card.check.reference-exact": "Совпадение с эталоном",
	"version-card.check.reference-similarity": "Сходство с эталоном",
	"version-card.check.turn-budget": "Длина диалога",
	"version-card.check.world-state": "Итоговое состояние мира",
	"version-card.check.cites-source": "Записанная проверка источника",
	"version-card.check.final-answer": "Наличие итогового ответа",
	"version-card.none-measured": "ничего не измерено",
	"version-card.more": "ещё {count}",
	"version-card.regressions": "Регрессы",
	"version-card.regression-summary": "регрессов по кейсам: {tasks} · новых типов сбоев: {newModes} · ухудшенных типов: {worsened} · целевых проблем не закрыто: {unresolved}",
	"version-card.resources": "Ресурсы",
	"version-card.absolute": "абсолютные значения",
	"version-card.ratios": "отношения",
	"version-card.cost": "цена",
	"version-card.latency": "задержка",
	"version-card.tokens": "токены",
	"version-card.change": "Правка",
	"version-card.paths": "Файлы",
	"version-card.artifacts": "Артефакты",
	"version-card.passport": "паспорт",
	"version-card.dataset": "датасет",
	"version-card.dialogues": "диалогов: {count}",
	"version-card.warnings": "Предупреждения",
	"release.html.report": "ОТЧЁТ О ВЕРСИИ",
	"release.html.before": "До",
	"release.html.after": "После",
	"release.html.delta": "Изменение балла",
	"release.html.confidence": "95% доверительный интервал",
	"release.html.sealed-note": "Экзамен отделён от разработки. Его вопросы остаются закрытыми.",
	"release.html.regressed-cases": "Регрессы по кейсам",
	"release.html.new-failures": "Новые типы сбоев",
	"release.html.unresolved": "Незакрытые проблемы",
	"release.html.worsened": "Ухудшенных типов сбоев: {count}",
	"release.html.total-cost": "Стоимость прогонов · до → после",
	"release.html.mean-latency": "Средняя задержка · до → после",
	"release.html.per-answer": "На один ответ, после / до",
	"release.html.exact-diff": "Посмотреть точную правку",
	"release.html.evidence-bound": "По сохранённым доказательствам этой версии",
	"release.html.artifacts-note": "Отчёт открывается сам по себе. Для скачивания файлов по ссылкам оставьте его в exports/ рядом с исходными артефактами проекта.",
	"release.html.revisions": "Точные ревизии · до → после",
	"release.html.offline": "Работает без интернета. Для сохранения в PDF используйте печать браузера.",
	"release.html.saved": "Отчёт для отправки: {path}",
	"release.html.not-saved": "Не удалось сохранить отчёт для отправки; доказательства версии доступны выше.",
	...
	"cmd.label": "Проверить судью: оценить его ответы вслепую, до {max} за раз",
	"cmd.err.tui": "/{command} работает только в окне Билдера",
	"cmd.err.no-args": "/{command} не принимает аргументов",
	"cmd.err.repetitions": "/{command} принимает число повторов — целое от 1 до 10",
	"cmd.err.regrade-id": "/regrade принимает id прогона из /traces, например /regrade erun_abc123",
	"cmd.err.branch": "в имени ветки можно только буквы, цифры и . _ - /",
	...
	"cmd.err.no-problem": "Проблемы {ordinal} нет; в /traces сейчас {count}.",
	"cmd.err.holdout-count": "Сколько кейсов — целое число; ты ввёл {answer}",
	"cmd.err.holdout-unavailable": "Загрузка закрытого экзамена в этом окне недоступна",
	"cmd.err.traces-rows": "/traces принимает число строк (до 60 на страницу), next или prev; например /traces 30",
	"cmd.err.review-page": "/review принимает next или prev для страниц кейсов кандидата, либо вызывается без аргументов",
	"cmd.review-no-cases": "Здесь нет доступного сравнения кандидата для просмотра по страницам. /review покажет текущий предмет проверки.",
	"cmd.review-no-more": "В этом направлении больше нет кейсов кандидата.",
	"cmd.err.target-arg": "/target принимает не больше одного пути ресурса агента",
	"cmd.err.passport-arg": "/passport принимает не больше одной версии, например /passport 0.2.0",
	"cmd.err.log-rows": "/log принимает число строк, например /log 10",
	"cmd.err.label-count": "/label принимает, сколько ответов оценить, например /label 20 (от 1 до {max})",
	"cmd.err.label-host": "/label нужен интерфейс, который умеет задавать вопросы; этот не умеет",
	"cmd.nothing-fixable": "Ни один тип сбоя пока не набрал данных, чтобы менять агента. Прогони тесты ещё раз или добавь кейсов.",
	"cmd.nothing-to-fix": "Чинить пока нечего — {next}",
	"cmd.ask-fix": "Скажи: «почини проблему {ordinal}».",
	"cmd.passport-not-writable": "Эта папка только для чтения, поэтому рядом с агентом ничего не сохранено.",
	"reason.interactive": "Запрошено вручную через /{command}",
	"model.auth-rejected": "Модель Билдера не приняла ключ. Скажи «подключи модель», настрой заново и повтори.",
	"model.rate-limited": "Модель Билдера упёрлась в лимит. Подожди и повтори — или скажи «подключи модель», выберем другую.",
	"model.unreachable": "Модель Билдера недоступна. Проверь сеть — или скажи «подключи модель», выберем другую.",
	"model.failed": "Модель Билдера не ответила. Повтори — или скажи «подключи модель», настроим заново.",
	"model.not-found": "Модель Билдера или её эндпоинт не найдены (404). Скажи «подключи модель» и выбери другую модель или провайдера.",
	"header.model-not-connected": "Модель Билдера не подключена",
	"onboarding.folder-not-empty": "В этой папке уже лежит {found}, поэтому создать агента здесь нельзя. Открой пустую папку и начни заново.",
	"onboarding.folder-unusable": "В этой папке агент жить не может — её нет или это не обычная папка. Открой обычную пустую папку проекта.",
	"onboarding.setup-stopped": "Настройка остановлена. Скажи, когда захочешь выбрать модель агента.",
	"onboarding.setup-failed": "Настройка не завершилась: {reason}",
	"onboarding.credential-name-only": "Назови ИМЯ переменной окружения, в которой лежит ключ, а не сам ключ.",
	"extension.no-shell": "У Билдера здесь нет шелла. Скажи словами, что нужно сделать, — он сделает это своими инструментами.",
	"blocker.target-missing": "Агент ещё не создан.",
	"blocker.target-placeholder": "У агента ещё стоят подставные имя и модель из шаблона.",
	"blocker.candidates-ambiguous": "Этому проекту подходят {candidates} — выбери одного.",
	"blocker.interrupted-candidate": "Прерванную попытку надо сбросить, прежде чем начинать новую.",
	"blocker.terminal-candidates": "{candidates} закончены, но ещё не приняты и не закрыты.",
	"blocker.applied-without-evidence": "{proposals} применены, но данных по ним ещё нет.",
	"blocker.proposals-await": "{proposals} ждут решения.",
	"blocker.spec-drafts": "{drafts} ждут проверки.",
	"blocker.approved-specs": "Есть {specs} — выбери, с каким продолжить.",
	"blocker.corpus-drafts": "{drafts} подходят одобренному описанию — выбери один.",
	...
	"refusal.target-not-ready": "Агент ещё не готов. {next}",
	"artifact.approved-spec": "одобренное описание",
	...
	"artifact.candidate": "кандидат",
	"view.resource.instructions": "инструкции",
	"view.resource.tool-executable": "программа инструмента",
	"view.resource.tool-descriptor": "описание инструмента",
	"view.resource.skill": "скилл",
	"view.resource.harness-file": "файл харнесса",
	...
	"confirm.improve.target-subtotal": "Подытог агента",
	"confirm.improve.builder-ceiling": "Лимит Билдера",
...
только один не выйдет дороже обычного, тогда будет один да/нет.`,
	...
	"why.expected.explicit-citation": "ожидалась явная ссылка на {source}",
	"why.actual.explicit-citation": "в конечном ответе нет точного id источника; совпадение текста не является ссылкой",
	...
	"refusal.regression-guards-broken": "Правка ломает {broken} из {guarded} кейсов разработки, которые базовая версия проходила всегда. Почини её или проверь ещё раз с большим числом повторов: со сломанным регрессионным кейсом выкатить нельзя.",
	"refusal.ship-check-regressed": "Проверка показала регресс на корзине разработки; такую правку выкатить нельзя. Почини её или отклони.",
	...
	// lane: the basket — coverage, the critic, the reading after a run, the simulator
	"mode.title.output-excludes": "В ответе есть то, чего быть не должно",
	"job.label.critique": "проверка кейсов",
	"confirm.title.critique-corpus": "Проверить кейсы судьёй",
	"confirm.critique-corpus": "Дать судье прочитать {cases} на валидность ({calls})? Он видит кейсы и их источники, ответы агента не видит.",
	"panel.critic": "Проверка кейсов",
	"critic.summary": "{valid} валидных · {repair} на правку · {invalid} невалидных · {unreviewed} не проверено",
	"critic.verdict.valid": "валиден",
	"critic.verdict.repair": "на правку",
	"critic.verdict.invalid": "невалиден",
	"critic.verdict.unreviewed": "не проверен",
	"critic.none": "критик этот черновик ещё не читал",
	"critic.next": "Поправь кейсы на правку, невалидные исключи с причиной, потом тесты.",
	"critic.exam-dropped": "{dropped} сгенерированных кейсов не прошли критика и не запечатаны: {reasons}",
	"refusal.publish-invalid-cases": "Критик пометил {invalid} кейсов как невалидные. Почини их или исключи с причиной; публикация с принудительным флагом перекроет это под запись.",
	"coverage.title": "Покрытие",
	"coverage.row": "{job} — {cells}",
	"coverage.empty": "пусто: {cells}",
	"coverage.unlabelled": "{cases} без метки покрытия",
	"coverage.unknown-job": "не из описания агента",
	"coverage.origins": "происхождение: {real} реальных · {synthetic} синтетических · {unknown} без метки",
	"coverage.excluded": "исключено с причиной: {cases}",
	"coverage.exam": "покрытие: {labelled} с ячейкой · {unlabelled} без ячейки",
	"basket.title": "Корзина после прогона",
	"basket.saturated": "{cases} с проходом в каждом повторе — регрессионные проверки; следующая волна сложнее",
	"basket.failing-valid": "{cases} с провалом в каждом повторе, критик считает кейс корректным — это работа над агентом",
	"basket.failing-doubtful": "{cases} с провалом всегда, критик сомневается в тесте: почини критерии и условия, за провал не удалять",
	"basket.failing-unreviewed": "{cases} с провалом в каждом повторе, и кейс никто не читал — сначала критик, потом решения",
	"basket.unstable": "{cases} с разным исходом между повторами — шум, не вердикт",
	"basket.wave": "эта волна добавила {cases}, из них проваливаются {failing}",
	"basket.wave-saturated": "эта волна добавила {cases}, агент прошёл все: насыщение — следующая волна должна быть сложнее",
	"basket.origins": "происхождение: {real} реальных · {synthetic} синтетических · {unknown} без метки; реализм {realism}",
	"basket.realism.unverified": "не проверен — сопоставимых реальных кейсов ещё нет",
	"basket.realism.compared": "сопоставлен по {cells} сопоставимым ячейкам",
	"basket.origin-cell": "{cell}: реальные {real}, синтетика {synthetic} ({gap})",
	"basket.next-wave": "следующая волна: {targets}",
	"basket.next-wave.cells": "пустые ячейки {cells}",
	"basket.next-wave.modes": "нерешённые режимы {modes}",
	"basket.next-wave.harder": "сложнее по {jobs}",
	"basket.rule": "Кейс никогда не удаляется за провал; чинится или исключается с причиной только невалидный тест.",
	"noise.simulator": "шум симулятора: вторая сторона шла на {model}; эта полоса про модель пользователя, не про агента, и не является доказательством для выкатки",
	"confirm.calibrate.simulator": "Прогнать ту же ревизию против себя с альтернативной моделью пользователя {model} на второй стороне ({runs})?",
	"behavior.clear": "ясный",
	"behavior.vague": "размытый",
	"behavior.impatient": "нетерпеливый",
	"behavior.wrong-facts": "с неверными фактами",
	"behavior.changes-goal": "меняет цель",
	"behavior.multi-issue": "несколько вопросов",
	"behavior.terse": "немногословный",
	"behavior.non-native": "неноситель языка",
};
...
/**
 * Whether a runtime-built key (a Workbench blocker code, a verdict token) has a
 * form in the dictionary. Nothing outside i18n gets to hold the table itself.
 */
